...
class StockPortfolioEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        # memorize portfolio info each step
    ...
    def step(self, actions):
       ...
       #     plt.plot(df.daily_return.cumsum(),'r')
        ...
        #print("Previous weights:", previous_weights)
        allocation_change = np.abs(weights - previous_weights)
        transaction_costs = self.transaction_cost_pct * np.sum(allocation_change) * self.portfolio_value
        #print("Transaction costs:", transaction_costs)
        #print("Action:", self.actions_memory)
        # Update portfolio value based on the portfolio return
        old_portfolio_value = self.portfolio_value
        new_portfolio_value = self.portfolio_value * (1 + portfolio_return)-transaction_costs
        self.portfolio_value = new_portfolio_value

        net_daily_return = (new_portfolio_value / old_portfolio_value) - 1  # includes transaction costs
    
        # 9) Store net daily return (after fees) instead of raw return
        self.portfolio_return_memory.append(net_daily_return)
        self.date_memory.append(current_date)
        self.asset_memory.append(new_portfolio_value)
        self.actions_memory.append(weights)
        self.transaction_cost_memory.append(transaction_costs)
    
        # 10) Reward can also reflect net daily retur
        self.reward = net_daily_return * self.reward_scaling
        ...
        #print(f"New portfolio value: {new_portfolio_value}, Portfolio return: {net_daily_return}")
    ...
    def save_transaction_cost_memory(self):
        """
        Save the memory of transaction costs and corresponding dates into a DataFrame.
        """
        date_list = self.date_memory
        transaction_costs = self.transaction_cost_memory
        df_transaction_costs = pd.DataFrame({'date': date_list, 'transaction_costs': transaction_costs})
        return df_transaction_costs
